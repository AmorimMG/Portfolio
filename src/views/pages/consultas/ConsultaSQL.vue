...
const codes = ref(
  Array(length.value).fill({ nome: null, consulta: "", id: null })
);
...
const fetchTable = () => {
  ...
  RESTAPI.EnviaQuery(query)
    .then((res) => {
      ...
      headers.value =
        res.data.length > 0
          ? Object.keys(res.data[0]).map((key) => ({ value: key, text: key }))
          : [];
...
<template>
  <div class="grid">
    <div style="width: 100%" class="container-main flex col-11 flex-row">
      <div
        style="margin-top: 40px; width: 50px"
        class="col-1 flex flex-column gap-6 align-items-center"
      >
        <div class="flex flex-column gap-1">
          ...
          <Button
            ...
            severity="info"
            raised
            @click="openLoadConfig()"
          />
          <Button
            icon="pi pi-pencil"
            :disabled="validaHabilitado"
            severity="secondary"
            raised
      ...
      <div style="width: 95%">
        <div class="container-child flex">
          <Tabs value="0" scrollable class="tab-container">
            <Tab
              ...
              :header="codes[index].nome ?? 'Script ' + (index + 1)"
              :value="index"
              @click="setCurrentTab(index)"
            >
              <span style="width: 60px">{{
                codes[index].nome ?? "Script " + (index + 1)
              }}</span>
              <Button
                style="width: 10px; margin-left: 15px"
                icon="pi pi-times"
                class="button-close button-danger"
        ...
        <splitter style="height: 450px" layout="vertical">
          <SplitterPanel class="flex items-center justify-center">
            <codemirror
              v-model="codes[currentTab].consulta"
              :options="cmOptions"
              :style="{ height: '100%' }"
              :autofocus="true"
  ...
  >
    ...
    <DataTable
      ...
      :currentPageReportTemplate="`${$t(
        'gblMostrando'
      )} {currentPage} ${$t('gblEm')} {totalPages} ${$t(
        'gblPaginas'
      )} ${$t('gblDe')} {totalRecords} ${$t(
        'gblResultados'
      )}`"
      responsiveLayout="scroll"
    >
  ...
  >
    ...
    <div class="popup-content flex flex-column gap-2">
      ...
      <FloatLabel>
        <InputText
          id="descricaoScript"
          v-model="script.descricaoScript"
          style="width: 100%"
        />
        <label for="descricaoScript">{{
          $t("gblDescricao")
        }}</label>
      </FloatLabel>
  ...
  >
    ...
    <div class="popup-content flex flex-column gap-2">
      ...
      <FloatLabel>
        <InputText
          id="descricao"
          v-model="codes[currentTab].descricao"
          style="width: 100%"
        />
        <label for="descricaoScript">{{
          $t("gblDescricao")
      ...
      <div class="field-checkbox mb-0">
        <input
          type="checkbox"
          id="ativo"
          v-model="codes[currentTab].padraoSistema"
        />
        <label for="padraoSistema">{{
          $t("gblPadraoSistema")
        }}</label>
  ...
  <Dialog
    v-model="loadConfigVisible"
    :visible="loadConfigVisible"
    @update:visible="onHide()"
    :modal="true"
    :closable="true"
    style="width: 500px"
  >
    ...
    <div class="popup-content flex flex-column gap-5">
      ...
      />
      <div
        class="inline-flex align-items-end flex gap-2"
