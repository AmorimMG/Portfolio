...
export const useFileSystemStore = defineStore("fileSystem", () => {
  ...
  async function detectFilesInCategory(category) {
    ...
    if (category === "images") {
      ...
      return cdnImages.map((imageData) => ({
        name: imageData.name,
        url: imageData.src, // getCdnImages usa 'src', não 'url'
        extension: imageData.extension,
        size: 0, // Tamanho não disponível do CDN
        modified: new Date(),
        id: imageData.id,
    ...
    // Para outras categorias, manter lógica anterior
    loadRegisteredFiles();
  ...
  /**
   * Função para obter imagens do CDN
   */
  async function loadCdnImagesData() {
    ...
    try {
      ...
      console.warn("⚠️ Fetch falhou, usando import dinâmico:", error);
      try {
        ...
        console.error("❌ Erro ao carregar dados do CDN:", importError);
  ...
  function getCdnImages() {
    if (!cdnImagesCache.value) {
      console.warn(
        "⚠️ Dados do CDN não carregados ainda, use loadCdnImagesData() primeiro"
      );
      return [];
    }

    if (!cdnImagesCache.value.images) {
      console.error("❌ Estrutura inválida nos dados do CDN");
      return [];
    }
