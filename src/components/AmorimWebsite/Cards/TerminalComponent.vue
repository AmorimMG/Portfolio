...
const processAnsiResponse = (response) => {
  if (typeof response === "string" && response.includes("\u001b[")) {
    ...
    setTimeout(() => {
      ...
      if (terminalContent) {
        const commands = terminalContent.querySelectorAll(
        ...
        if (lastCommandContainer) {
          ...
          setTimeout(() => {
            const responseElement = lastCommandContainer.querySelector(
              '[data-pc-section="response"]'
            );
            if (responseElement) {
...
const formatTestimonials = () => {
  return terminalConfig.testimonials
    .map((t) => `"${t.content}" - ${t.author}, ${t.role} at ${t.company}`)
    .join("\n");
};

const formatFAQs = () => {
  return terminalConfig.faqs
...
Use 'tree' to see directory structure.`;
    ...
    case "pt":
      changeLanguage("pt");
      response = computed(() =>
        t("Terminal.LanguageChanged", { language: "Português" })
      );
      break;
    case "es":
      changeLanguage("es");
      response = computed(() =>
        t("Terminal.LanguageChanged", { language: "Español" })
      );
      break;
    case "en":
      changeLanguage("en");
      response = computed(() =>
        t("Terminal.LanguageChanged", { language: "English" })
      );
      break;
  ...
  <div id="Terminal" class="col-span-4 lg:col-span-4 xl:col-span-4 pb-0">
    <CardEffect class="card-effect" v-if="props.isCard">
      <div class="terminal-card" style="width: 100%; height: 100%; padding: 0">
        <Terminal
          :welcomeMessage="
            'Welcome to Amorim Terminal. ' +
            'Type \'help\' for portfolio commands or try Unix commands like \'ls\', \'cd\', \'cat\''
          "
