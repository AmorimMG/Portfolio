      ...
      <div class="content-page settings-page" ref="settingsPage">
        ...
        <div class="theme-section" style="margin-top: 25px;">
          ...
          <div class="language-list">
            <button v-for="lang of languageOptions" :key="lang.value"
              @click="changeLanguage(lang)"
              :class="['language-button', { 'active': currentLanguage?.value === lang.value }]">
              <img
                loading="lazy"
                :alt="lang.name"
              ...
              <i v-if="currentLanguage?.value === lang.value" class="pi pi-check"></i>
            </button>
          </div>
        </div>
      </div>
      <div class="content-page theme-page" ref="themePage">
        ...
        <div class="theme-section">
          ...
          <div class="color-grid">
            <button v-for="primaryColor of primaryColors" :key="primaryColor.name" type="button"
              ...
              :class="['color-button', { 'active': layoutConfig.primary === primaryColor.name }]"
        ...
        <div class="theme-section">
          ...
          <div class="color-grid">
            <button v-for="surface of surfaces" :key="surface.name" type="button"
              ...
              :class="['color-button', { 
                'active': layoutConfig.surface ? layoutConfig.surface === surface.name : isDarkTheme ? surface.name === 'zinc' : surface.name === 'slate' 
              }]"
        ...
        <div class="theme-section">
          ...
          <div class="preset-buttons">
            <button v-for="presetOption of presetOptions" :key="presetOption"
...
import { getLanguageCookie, setColorThemeCookie, setLanguageCookie } from '@/service/session.js';
...
function changePreset(presetValue) {
    ...
    $t().preset(selectedPreset).preset(getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
}
...
watch(locale, (newLocale) => {
  const newLanguage = languageOptions.value.find(lang => lang.value === newLocale);
  if (newLanguage) {
...
onMounted(() => {
  ...
  if (savedLanguage) {
    currentLanguage.value = languageOptions.value.find(lang => lang.value === savedLanguage);
  } else {
    currentLanguage.value = languageOptions.value.find(lang => lang.value === 'en');
  }
