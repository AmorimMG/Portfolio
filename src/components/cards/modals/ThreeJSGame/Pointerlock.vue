...
function ThirdPerson() {
    // CAMERA
    const cameraFP = new THREE.PerspectiveCameraFP(45, window.innerWidth / window.innerHeight, 0.1, 1000);
    cameraFP.position.y = 5;
    cameraFP.position.z = 5;
    cameraFP.position.x = 0;
    cameraFP.lookAt(new THREE.Vector3(0, 0, 0));

    // CONTROLS
    const orbitControls = new OrbitControls(cameraFP, renderer.domElement);
    ...
    if (canRender.value === true) {
        new GLTFLoader().load('/src/assets/images/threejs/models/Soldier.glb', function (gltf) {
            ...
            characterControls = new CharacterControls(model, mixer, animationsMap, orbitControls, cameraFP, 'Idle');
        });
    } else {
